...
test('Avodah presents the required research and two cumulative build layers', () => {
  ...
  assert.match(proposal, /\$10,500/)
  assert.match(proposal, /\$8,000/)
  assert.match(proposal, /\$14,000/)
  assert.match(proposal, /\$18,500/)
  assert.equal((proposal.match(/Selected \$8,000 engagement: 50% at project kickoff \(\$4,000\), then 50% at project completion \(\$4,000\)\./g) ?? []).length, 2)
  assert.match(proposal, /Pay \$4,000 at kickoff and \$4,000 at completion/)
  assert.match(proposal, /\$5,250/)
  assert.match(proposal, /50% at site kickoff/)
  assert.doesNotMatch(proposal, /75% at kickoff/)
  assert.doesNotMatch(proposal, /50% at implementation kickoff/)
  assert.doesNotMatch(proposal, /\$875/)
  assert.doesNotMatch(proposal, /\$2,250/)
  assert.doesNotMatch(proposal, /\$1,125/)
  assert.match(proposal, /Search Foundation is required/)
  assert.match(proposal, /complete program is \$18,500/)
  ...
  assert.doesNotMatch(proposal, /\$5,000/)
  assert.doesNotMatch(proposal, /\$11,000/)
  assert.doesNotMatch(proposal, /\$19,500/)

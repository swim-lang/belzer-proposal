...
test('Bennett contract preserves proposal economics and enables authorized signing', () => {
  const data = readFileSync('src/contracts/bennettContract.ts', 'utf8')
  for (const expected of ['$39,500', '$19,750', '$9,875', '$3,000', '$13,000', '$14,000', '$6,500', '5 to 7 weeks', 'Missoula', 'associated agency travel', 'identity presentation and approval']) assert.ok(data.includes(expected), expected)
  ...
  assert.ok(data.includes("agencySignedDate: 'September 16, 2026'"))
  assert.ok(data.includes('depositHref:'))
  assert.ok(!data.includes('draftOnly: true'))

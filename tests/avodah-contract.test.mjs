...
test('Avodah contract reflects the approved first two phases only', () => {
  ...
  assert.equal((contract.match(/amount: '\$4,000'/g) ?? []).length, 2)
  assert.match(contract, /Due at project kickoff/)
  assert.match(contract, /Due at project completion/)
  assert.match(contract, /completed the full contracted scope/)
  assert.doesNotMatch(contract, /amount: '\$2,625'/)
  assert.doesNotMatch(contract, /amount: '\$875'/)
  assert.doesNotMatch(contract, /amount: '\$2,250'/)
  assert.doesNotMatch(contract, /amount: '\$1,125'/)

    ...
    <section className="bg-ink text-paper border-b border-[var(--color-rule)] px-6 md:px-16 lg:px-[120px] py-24 lg:py-[160px]">
      <Reveal className="flex items-start justify-between pb-8 border-b border-white/20 gap-6">
        <div className="flex flex-col gap-1.5">
          <span className="eyebrow text-paper/66">{phaseTwo.eyebrowSection}</span>
          <span className="eyebrow text-paper">{phaseTwo.eyebrowEmphasis}</span>
        </div>
        <span className="eyebrow text-paper/66 text-right">{phaseTwo.metaRight}</span>
      </Reveal>
      <div className="flex flex-col lg:flex-row lg:items-end gap-12 lg:gap-20 py-12 lg:py-20">
        ...
        <Reveal className="flex flex-col gap-5 flex-1 max-w-[480px] pb-6">
          <p className="text-[16px] leading-[26px] text-paper/78">{phaseTwo.body}</p>
        </Reveal>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] border-t border-b border-white/20">
        <Reveal className="flex flex-col gap-6 py-10 lg:pr-12 lg:border-r border-white/20">
          <span className="eyebrow text-paper/66">{phaseTwo.buildPathsLabel}</span>
          <div>
            {buildPaths.map((b, i) => (
              <div
                key={i}
        ...
        <Reveal className="flex flex-col gap-8 py-10 lg:pl-12">
          <div className="flex flex-col gap-3">
            <span className="eyebrow text-paper/66">{phaseTwo.rangeLabel}</span>
          ...
          <p className="text-[14px] leading-[22px] text-paper/66">{phaseTwo.rangeNote}</p>
          <p className="text-[12px] leading-[20px] text-paper/48 tracking-[0.02em]">{phaseTwo.rangeDisclaimer}</p>
        </Reveal>
      </div>
    </section>
  )

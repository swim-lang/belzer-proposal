...
import { SyncBadge } from './AdminChrome'
    ...
    <div className="flex flex-col gap-2 px-5 py-4 border-b border-[var(--color-rule)]/20 bg-paper sticky top-0 z-10">
      <div className="flex items-center justify-between gap-3">
        <div className="flex flex-col gap-0.5">
          <span className="text-[11px] tracking-[0.12em] uppercase text-ink-2">Editor</span>
          <span className="text-[13px]">
            <span className="font-medium">{content.client.name}</span>
          </span>
        </div>
  ...
  return (
    <div className="flex flex-col px-5 pb-16">
      ...
      <Section title="Approve Takeover Screen">
        ...
        <TextField path="approveScreen.ctaIntake" label="Intake CTA" />
        <TextField path="approveScreen.intakeHref" label="Intake URL" />
      </Section>
    </div>
  )
}
...
export function AdminEditor() {
  return (
    <div className="flex-1 grid min-h-0" style={{ gridTemplateColumns: '440px 1fr' }}>
      <aside className="border-r border-[var(--color-rule)]/20 flex flex-col overflow-hidden">
        <Toolbar />
